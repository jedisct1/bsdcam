...
struct tree_node {
	short left;		/* Index of left child node */
	short right;		/* Index of right child node */
...
get_qt_dynamic(unsigned char *pIn, struct comp_info *cinfo)
{
	...
	if (rawLen < 72) {
		fprintf(stderr, "Not enough input to decompress\n");
		return -EINVAL;
	}
